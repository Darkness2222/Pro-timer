...
export default function ReportsPage({
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        // Fetch all events for the organization (including soft-deleted)
        ...
        // Fetch timers for each event
        const eventsWithTimers = await Promise.all(
          (eventsData || []).map(async (event) => {
            const { data: eventTimers } = await supabase
              .from('timers')
              .select('*')
            ...
            const { data: logs } = await supabase
              .from('timer_logs')
              .select('*')
              .in('timer_id', (eventTimers || []).map(t => t.id))
  ...
  const getEventStats = (event) => {
    ...
    completedTimers.forEach(timer => {
      const finishLog = event.logs.find(log =>
        log.timer_id === timer.id &&
        (log.action === 'finished' || log.action === 'expired')
      )

      const overtimeLogs = event.logs.filter(log =>
        log.timer_id === timer.id &&
        (log.action === 'overtime' || log.action === 'expired' ||
         (log.overtime_seconds !== null && log.overtime_seconds > 0))
      )
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-2">
                      <div className="text-center">
                        <div className="text-green-400 font-bold text-lg">{stats.early}</div>
                        <div className="text-gray-500 text-xs">Early</div>
                      </div>
                      <div className="text-center">
                        <div className="text-blue-400 font-bold text-lg">{stats.onTime}</div>
                        <div className="text-gray-500 text-xs">On Time</div>
                      </div>
                      <div className="text-center">
                        <div className="text-red-400 font-bold text-lg">{stats.overtime}</div>
                        <div className="text-gray-500 text-xs">Overtime</div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {metrics.totalCompleted > 0 && (
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700 mb-8">
          ...
          <div className="mt-6">
            ...
            <div className="relative h-64 flex items-end gap-4 bg-gray-900/50 rounded-lg p-6">
              <div className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex flex-col justify-end" style={{ height: '200px' }}>
                  <div
                    ...
                    style={{ height: `${(metrics.earlyFinishCount / metrics.totalCompleted) * 100}%` }}
                  >
                    <span className="text-white font-bold text-sm">{metrics.earlyFinishCount}</span>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex flex-col justify-end" style={{ height: '200px' }}>
                  <div
                    ...
                    style={{ height: `${(metrics.onTimeCount / metrics.totalCompleted) * 100}%` }}
                  >
                    <span className="text-white font-bold text-sm">{metrics.onTimeCount}</span>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex flex-col justify-end" style={{ height: '200px' }}>
                  <div
                    ...
                    style={{ height: `${(metrics.overtimeCount / metrics.totalCompleted) * 100}%` }}
                  >
                    <span className="text-white font-bold text-sm">{metrics.overtimeCount}</span>
                  </div>
                </div>
